...
Advantage Estimation Methods:
  --advantage-type td   : 1-step TD advantage A = r + γV(s') - V(s) [High variance, sparse signal]
  --advantage-type gae  : GAE(λ) [Recommended - bias-variance tradeoff, smoother distribution]
  --advantage-type mc   : Monte Carlo A = G_t - V(s) [Uses full return, more stable]

Additional Options:
  --normalize-advantages : Normalize advantages per episode (zero mean, unit variance)
  --advantage-clip N     : Clip advantages to [-N, +N] range
  --gae-lambda 0.95      : Lambda for GAE (higher = more MC-like, lower = more TD-like)

The output JSONL includes additional fields:
  - "value": V(s) from MAPPO critic
  - "q_value": Estimated Q(s,a) = r + γV(s')
  - "advantage": A(s,a) computed via selected method
  - "advantage_type": Method used ("td", "gae", or "mc")
...
def get_learner_module(trainer: PPO, policy_name: str):
    """
    Get the full RLModule from learner (with critic) instead of inference-only module.
    The learner's module contains both actor (pi) and critic (vf).
    """
    try:
        learner = trainer.learner_group._learner
        for name in [policy_name, "shared_pursuer", "shared_policy", "default_policy"]:
            if name in learner.module:
                return learner.module[name]
    except Exception as e:
        print(f"[Warning] Could not get learner module: {e}")
    
    # Fallback to inference module (won't have critic values)
    return trainer.get_module(policy_name)


def compute_action_and_value(module, obs: np.ndarray, global_state: np.ndarray) -> Tuple[int, float, np.ndarray]:
    """
    Compute action, value V(s), and action logits from MAPPO.
    
    Args:
        module: The RLModule (should be from learner to have critic)
        obs: Local observation for the agent
        global_state: Global state for centralized critic
    ...
    batch = {"obs": obs_tensor, "state": state_tensor}

    # Get action and logits via forward_inference
    ...
    # Get value from critic using compute_values (works on learner module)
    value = 0.0
    try:
        v = module.compute_values(batch)
        value = float(v[0].item() if hasattr(v, "ndim") and v.ndim > 0 else v.item())
    except Exception as e:
        # Fallback: try vf_preds from forward pass or value_function method
        try:
            if "vf_preds" in out:
                value = float(out["vf_preds"][0].item())
            elif hasattr(module, 'value_function'):
                v = module.value_function(batch)
                value = float(v[0].item() if hasattr(v, "ndim") and v.ndim > 0 else v.item())
        except:
            pass
...
def compute_returns_and_advantages(
    ...
    - q_value: Estimated Q(s,a) = r + γV(s')
    - advantage: A(s,a) computed via TD, GAE, or Monte Carlo
    
    Args:
        episode_data: List of transition records from one episode
        gamma: Discount factor
        gae_lambda: Lambda for GAE (only used if advantage_type="gae")
        advantage_type: "td" (1-step TD), "gae" (GAE-lambda), or "mc" (Monte Carlo)
        normalize_advantages: If True, normalize advantages per episode
        advantage_clip: If set, clip advantages to [-clip, +clip]
    """
    # Group by agent_id
    agent_data = {}
    ...
    # Compute returns and advantages for each agent's trajectory
    processed = []
    for aid, trajectory in agent_data.items():
        ...
        rewards = np.array([trajectory[t]["reward"] for t in range(T)])
        values = np.array([trajectory[t]["value"] for t in range(T)])
        ...
        if advantage_type == "mc":
            # Monte Carlo: A(s,a) = G_t - V(s)
            # More stable, uses full return instead of bootstrap
            advantages = returns - values
            
        elif advantage_type == "gae":
            # GAE(lambda): Weighted combination of n-step advantages
            # Provides bias-variance tradeoff controlled by lambda
            gae = 0.0
            for t in reversed(range(T)):
                if t + 1 < T:
        ...
        # Normalize advantages per episode (zero mean, unit std)
        if normalize_advantages and T > 1:
            ...
            if adv_std > 1e-8:
                ...
                advantages = advantages - adv_mean  # Just center if no variance
        
        # Clip advantages if specified
        if advantage_clip is not None:
            advantages = np.clip(advantages, -advantage_clip, advantage_clip)
        
        # Store computed values in records
        for t, rec in enumerate(trajectory):
            rec["return_to_go"] = float(returns[t])
            
            # Q-value (1-step estimate for reference)
            if t + 1 < T:
                next_value = values[t + 1]
            else:
                next_value = 0.0
            rec["q_value"] = float(rewards[t] + gamma * next_value)
            
            # Advantage from selected method
            rec["advantage"] = float(advantages[t])
            rec["advantage_type"] = advantage_type
...
def main():
    ...
    ap.add_argument("--gae-lambda", type=float, default=0.95, help="GAE lambda for advantage estimation")
    ap.add_argument("--advantage-type", type=str, default="gae", 
                    choices=["td", "gae", "mc"], 
                    help="Advantage estimation method: td=1-step TD, gae=GAE(lambda), mc=Monte Carlo")
    ap.add_argument("--normalize-advantages", action="store_true", 
                    help="Normalize advantages per episode (zero mean, unit variance)")
    ap.add_argument("--advantage-clip", type=float, default=None,
                    help="Clip advantages to [-clip, +clip] range (None=no clipping)")
    ...
    # Get the full module from learner (with critic) instead of inference-only module
    learner_module = get_learner_module(trainer, policy_name)
    print(f"[Info] Learner module type: {type(learner_module).__name__}")
    print(f"[Info] Has critic (vf): {hasattr(learner_module, 'vf')}")
    ...
    try:
        while episodes_collected < args.episodes:
                ...
                for aid, ob in obs.items():
                    ...
                    try:
                        action, value, logits = compute_action_and_value(
                            learner_module, ob, global_state
                        )
                    except Exception as e:
            ...
            processed_records = compute_returns_and_advantages(
                episode_records, 
                gamma=args.gamma,
            ...
            # Update advantage stats (track pos/neg ratio and std)
            ...
            if advantages:
                ...
                pos_count = sum(1 for a in advantages if a > 0)
                ...
                advantage_stats["std"] = new_std  # Update to latest episode std
                advantage_stats["positive"] = advantage_stats.get("positive", 0) + pos_count
    ...
    print(f"  Advantage type: {args.advantage_type}" + 
          (f" (λ={args.gae_lambda})" if args.advantage_type == "gae" else ""))
    print(f"  Normalize advantages: {args.normalize_advantages}")
    if args.advantage_clip:
        print(f"  Advantage clip: ±{args.advantage_clip}")
    print(f"  Avg advantage: {advantage_stats['mean']:.4f}")
    if advantage_stats["count"] > 0:
        pos_pct = 100 * advantage_stats.get("positive", 0) / advantage_stats["count"]
        neg_pct = 100 - pos_pct
        print(f"  Positive/Negative ratio: {pos_pct:.1f}% / {neg_pct:.1f}%")
    if episode_returns:
