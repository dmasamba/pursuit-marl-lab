...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (IPPO - ADDITIONAL EVADERS, OBS IMAGES)")
        print('='*60)
...
if __name__ == "__main__":
    ...
    # Generalization test: double the evaders (training uses 2, test uses 4)
    num_agents = 2
    num_evaders = 2
    x_size = 8
    y_size = 8
    ...
    output_dir = os.path.join(project_root, "artifacts", f"ippo_obs_images_eval_results/ippo_generalization_additional_evaders_obs_images_{current_time}")
    ...
    print(f"\n{'='*60}")
    print("IPPO GENERALIZATION TEST: ADDITIONAL EVADERS (OBS IMAGES)")
    print('='*60)
    print(f"Grid: {x_size}x{y_size}, Max cycles: {max_cycles}")
    print(f"Pursuers: {num_agents}, Evaders: {num_evaders} (2x training), n_catch: {n_catch}")
