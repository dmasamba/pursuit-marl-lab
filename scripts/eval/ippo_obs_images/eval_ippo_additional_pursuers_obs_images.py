...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (IPPO - ADDITIONAL PURSUERS, OBS IMAGES)")
        print('='*60)
...
if __name__ == "__main__":
    ...
    # Generalization test: more pursuers (training uses 4, test uses 6) with higher n_catch
    num_agents = 3
    num_evaders = 1
    x_size = 8
    y_size = 8
    ...
    module_ids = [f"pursuer_{i}" for i in range(num_agents)]
    module_assignment = {agent_id: agent_id for agent_id in module_ids}
    module_reuse_active = False
    ...
    cell_scale = inferred_cell_scale if inferred_cell_scale is not None else 24
    if inferred_cell_scale is not None:
        print(f"Using cell_scale={cell_scale} inferred from checkpoint.")
    elif checkpoint_path:
        print("No cell_scale found in checkpoint; defaulting to 24.")
    normalize = True
    draw_counts = True
    ...
    if checkpoint_path:
        ...
        module_ids = get_restored_module_ids(trainer)
        if not module_ids:
            raise RuntimeError(f"No policy modules were restored from checkpoint: {checkpoint_path}")
        requested_agent_ids = sort_agent_ids(f"pursuer_{i}" for i in range(num_agents))
        if len(module_ids) != num_agents:
            print(
                f"Checkpoint provides {len(module_ids)} policy modules {module_ids}; "
                f"reusing them cyclically across {num_agents} evaluation pursuers."
            )
        module_assignment = {
            agent_id: module_ids[idx % len(module_ids)]
            for idx, agent_id in enumerate(requested_agent_ids)
        }
        module_reuse_active = any(
            agent_id != module_id
            for agent_id, module_id in module_assignment.items()
        )
    else:
    ...
    output_dir = os.path.join(project_root, "artifacts", f"ippo_obs_images_eval_results/ippo_generalization_additional_pursuers_obs_images_{current_time}")
    os.makedirs(output_dir, exist_ok=True)

    config = {
        "checkpoint_path": checkpoint_path, "requested_num_agents": requested_num_agents,
        "num_agents": num_agents, "module_ids": module_ids,
    ...
    print(f"\n{'='*60}")
    print("IPPO GENERALIZATION TEST: ADDITIONAL PURSUERS (OBS IMAGES)")
    print('='*60)
    print(f"Grid: {x_size}x{y_size}, Max cycles: {max_cycles}")
    print(f"Pursuers: {num_agents}, Evaders: {num_evaders}, n_catch: {n_catch}")
    if checkpoint_path and module_reuse_active:
        print(f"Module reuse map: {module_assignment}")
    ...
    for ep in range(num_eval_episodes):
        ...
        expected_agent_ids = sort_agent_ids(module_assignment.keys())
        if env_agent_ids != expected_agent_ids:
            raise RuntimeError(
                f"Environment agent IDs {env_agent_ids} do not match expected evaluation agent IDs {expected_agent_ids}."
            )
        ...
        while not done["__all__"]:
            ...
            for agent_id, obs in obs_dict.items():
                ...
                module_id = module_assignment[agent_id]
                agent_module = trainer.env_runner_group.local_env_runner.module[module_id]
