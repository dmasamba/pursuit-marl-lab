#!/usr/bin/env python
"""Evaluate an IPPO checkpoint with the ego agent shifted off-center in observations.

During training the ego agent is always rendered at the exact center of the
observation window (h//2, w//2). This test shifts the agent to a different
position within the SAME canvas size by sliding the obs content: context on the
opposite side is zero-padded and context on the shift side is cropped out.

The CNN input resolution is intentionally preserved so that only the spatial
position of the ego is disturbed — the shift is isolated from any resolution
mismatch.

A model that hard-coded "I am at the center" will misread relative distances
and directions to the evader. A model with true spatial reasoning (e.g. a VLM
that reads the blue marker wherever it appears) is unaffected.
"""
...
def _shift_obs_canvas(
    obs: np.ndarray,
    ego_row_offset: int,
    ego_col_offset: int,
) -> Tuple[np.ndarray, Tuple[int, int]]:
    """Slide the obs content within the same canvas so ego appears off-center.

    The raw obs has the ego agent at (H//2, W//2). After shifting by
    (ego_row_offset, ego_col_offset) the ego appears at
    (H//2 + ego_row_offset, W//2 + ego_col_offset).

    Negative offsets move the ego toward the top-left; positive toward
    bottom-right. The canvas size is unchanged — content is zero-padded on
    the side opposite to the shift direction.

    Returns:
        shifted_obs: same shape as obs, ego at new position.
        ego_pos: (row, col) of the ego in the shifted canvas.
    """
    H, W, C = obs.shape
    canvas = np.zeros_like(obs)
    # Channel 0 represents walls/boundaries in pursuit observations.
    # Out-of-bounds areas are 1.0 (walls) during training, so pad with 1.0
    # to preserve visual semantics instead of introducing false open space.
    canvas[:, :, 0] = 1.0

    r = ego_row_offset
    c = ego_col_offset

    # canvas[i, j] = obs[i - r, j - c]  (when in bounds)
    src_r0 = max(0, -r)
    src_r1 = min(H, H - r)
    dst_r0 = max(0, r)
    dst_r1 = min(H, H + r)
    ...
    if src_r1 > src_r0 and src_c1 > src_c0:
        canvas[dst_r0:dst_r1, dst_c0:dst_c1] = obs[src_r0:src_r1, src_c0:src_c1]

    ego_row = H // 2 + r
    ego_col = W // 2 + c
    return canvas, (ego_row, ego_col)


class NonCenteredEgoObservationImageRenderer:
    """Render pursuit observations with the ego marker at an explicit position.

    Colors are kept identical to training so the only perturbation is the
    spatial position of the ego within the observation window.
    """
    ...
    def render(
        ...
        """Render obs as an RGB image with the ego marker at ego_pos.

        Args:
            obs: shifted observation array (H, W, C).
            ego_pos: (row, col) of the ego agent in the canvas — NOT h//2, w//2.
        """
        ...
        # Draw ego at the explicitly provided position (not h//2, w//2).
        ci, cj = ego_pos
...
class NonCenteredEgoObservationImageWrapper(BaseParallelWrapper):
    """Replace observations with ego-shifted RGB images and attach state.

    The raw obs (agent at center) is preserved in info["raw_obs"] for metrics.
    The shifted canvas (ego off-center) is what the model receives as obs.
    """
    ...
    def __init__(
        ...
        *,
        ...
        ego_row_offset: int = -2,
        ego_col_offset: int = -2,
    ):
        super().__init__(env)
        ...
        self.ego_row_offset = ego_row_offset
        self.ego_col_offset = ego_col_offset
        self._agent_ids = [f"pursuer_{i}" for i in range(num_agents)]

        sample_obs, _ = self.env.reset()
        sample_agent = next(iter(sample_obs))
        sample_cropped = self._maybe_crop(sample_obs[sample_agent])
        sample_shifted, sample_ego_pos = _shift_obs_canvas(
            sample_cropped, ego_row_offset, ego_col_offset
        )
        sample_img = self.renderer.render(sample_shifted, sample_ego_pos)
        if normalize:
    ...
    def _maybe_crop(self, local_obs: np.ndarray) -> np.ndarray:
        """Center crop or pad to target logical grid size before ego shifting."""
    ...
    def _render_agent_obs(
        self, agent_obs: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, Tuple[int, int]]:
        """Returns (rendered_image, raw_cropped_obs, ego_pos_in_shifted_canvas)."""
        cropped = self._maybe_crop(agent_obs)
        shifted, ego_pos = _shift_obs_canvas(cropped, self.ego_row_offset, self.ego_col_offset)
        img = self.renderer.render(shifted, ego_pos)
        if self.normalize:
            img = (img.astype(np.float32) / 255.0).copy()
        else:
            img = img.copy()
        return img, cropped, ego_pos

    def _stack_state(self) -> np.ndarray:
        return np.stack(
            [self._last_images.get(aid, self._zero_image) for aid in self._agent_ids],
            axis=0,
        )
    ...
    def reset(
        ...
        ego_positions: Dict[str, Tuple[int, int]] = {}

        for agent, agent_obs in obs.items():
            img, cropped, ego_pos = self._render_agent_obs(agent_obs)
            obs_imgs[agent] = img
            raw_obs[agent] = cropped
            ego_positions[agent] = ego_pos
        ...
        for agent in obs_imgs:
            ...
            info_dict[agent]["ego_pos"] = ego_positions[agent]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate IPPO checkpoint with the ego agent shifted off-center in the "
            "observation image. The canvas size is unchanged from training — obs content "
            "is slid within the canvas so the ego appears at a different spatial position. "
            "Colors are identical to training."
        )
    )
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of pursuer agents (independent policies).",
    )
    parser.add_argument(
        "--num-evaders",
        "--num_evaders",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Pixel size per grid cell. Inferred from checkpoint if available; otherwise 24.",
    )
    parser.add_argument(
        "--ego-row-offset",
        "--ego_row_offset",
        ...
        default=-2,
        help=(
            "Row offset for the ego position relative to the canvas center. "
            "Negative = shift toward top, positive = shift toward bottom (default: -2)."
        ),
    )
    parser.add_argument(
        "--ego-col-offset",
        ...
        default=-2,
        help=(
            "Column offset for the ego position relative to the canvas center. "
            "Negative = shift toward left, positive = shift toward right (default: -2)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "(default: artifacts/ippo_obs_images_eval_results/ippo_non_centered_ego_obs_images_<timestamp>).",
    )
    parser.add_argument(
        "--obs-crop-size",
        "--obs_crop_size",
        type=int,
        default=None,
        help="Optional target grid size (cells) applied before ego shifting. 0 or unset disables.",
    )
...
def make_env(
    ...
    env = NonCenteredEgoObservationImageWrapper(
        ...
        ego_row_offset=ego_row_offset,
        ego_col_offset=ego_col_offset,
    )
...
def moves_toward_evader(obs: np.ndarray, action: int) -> bool:
    """Computed on the unshifted raw_obs — ego is at true center."""
    h, w = obs.shape[:2]
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str, ego_row_offset: int, ego_col_offset: int):
        summary = self.compute_summary()
        with open(path, "w") as f:
            json.dump(summary, f, indent=2)
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (IPPO - NON-CENTERED EGO)")
        print("=" * 60)
        print(f"Ego shift: row={ego_row_offset:+d}, col={ego_col_offset:+d} cells from center")
        print(f"  (negative = toward top-left, positive = toward bottom-right)")
...
def main():
    ...
    if args.cell_scale is None and inferred_cell_scale is not None:
        ...
        print("No cell_scale provided and none found in checkpoint; defaulting to 24 (matches training).")
    ...
    tune.register_env("pursuit_env_ippo_obs_images_non_centered_ego", env_creator)
    ...
    output_dir = (
        args.output_dir
        or os.path.join(project_root, f"ippo_obs_images_eval_results/ippo_non_centered_ego_obs_images_{current_time}")
    )
    ...
    config = {
        ...
        "ego_row_offset": args.ego_row_offset,
        "ego_col_offset": args.ego_col_offset,
        "seed": args.seed,
        "num_eval_episodes": args.episodes,
    }
    ...
    print(f"\n{'='*60}")
    print("IPPO GENERALIZATION TEST: NON-CENTERED EGO")
    print("=" * 60)
    print(f"Perturbation: ego shifted {args.ego_row_offset:+d} rows, {args.ego_col_offset:+d} cols from canvas center")
    print(f"  Training: ego always at center (h//2, w//2)")
    print(f"  This test: ego at (h//2 + {args.ego_row_offset}, w//2 + {args.ego_col_offset})")
    print(f"  Canvas size: unchanged — content slides, zero-padding fills the gap")
    print(f"  Colors: identical to training (only position is perturbed)")
    ...
    if obs_crop_size:
        print(f"Center-cropping raw obs to {obs_crop_size}x{obs_crop_size} before ego shift.")
    else:
    ...
    for ep in range(num_eval_episodes):
        ...
        first_agent = env_agent_ids[0]
        initial_distance = get_initial_evader_distance(info[first_agent]["raw_obs"])

        while not done["__all__"]:
            action_dict = {}
            for agent_id, obs in obs_dict.items():
                # Metrics use raw_obs (unshifted, ego at true center).
                # The model receives the shifted image obs.
    ...
    metrics_agg.save_summary(summary_path, args.ego_row_offset, args.ego_col_offset)
