#!/usr/bin/env python
"""Evaluate an IPPO checkpoint with semantically swapped agent colors.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate IPPO checkpoint with semantically swapped pursuer/evader colors. "
    ...
    parser.add_argument(
        ...
        "(default: ippo_obs_images_eval_results/ippo_semantic_color_swap_obs_images_<timestamp>).",
    )
    parser.add_argument(
        "--pursuer-color",
        "--pursuer_color",
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (IPPO - SEMANTIC COLOR SWAP)")
        print("=" * 60)
...
def main():
    ...
    tune.register_env("pursuit_env_ippo_obs_images_semantic_color_swap", env_creator)
    ...
    output_dir = (
        args.output_dir
        or os.path.join(project_root, "artifacts", f"ippo_obs_images_eval_results/ippo_semantic_color_swap_obs_images_{current_time}")
    )
    ...
    print(f"\n{'='*60}")
    print("IPPO GENERALIZATION TEST: SEMANTIC COLOR SWAP")
    print("=" * 60)
