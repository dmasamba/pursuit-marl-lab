#!/usr/bin/env python
"""Evaluate an IPPO checkpoint with shape-shifted observation-built images.

This script mirrors eval_ippo_obs_images.py but renders pursuers and
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate IPPO checkpoint with shape-shifted observation images."
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
    ...
    parser.add_argument(
        ...
        "(default: ippo_obs_images_eval_results/ippo_shape_shift_obs_images_<timestamp>).",
    )
    parser.add_argument(
        "--background-color",
        "--background_color",
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (IPPO - SHAPE-SHIFTED OBS IMAGES)")
        print("=" * 60)
...
def main():
    ...
    tune.register_env("pursuit_env_ippo_obs_images_shape_shift", env_creator)
    ...
    output_dir = args.output_dir or os.path.join(project_root, "artifacts", f"ippo_obs_images_eval_results/ippo_shape_shift_obs_images_{current_time}")
    ...
    print(f"\n{'='*60}")
    print("IPPO GENERALIZATION TEST: SHAPE-SHIFTED OBS IMAGES")
    print("=" * 60)
    print(
