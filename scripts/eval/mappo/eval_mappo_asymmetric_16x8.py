...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - ASYMMETRIC GRID 16x8)")
        print('='*60)
...
if __name__ == "__main__":
    # HARD Generalization Test: Asymmetric Grid (16x8 - Rectangular!)
    num_agents = 2
    num_evaders = 1
    x_size = 16  # Wide
    y_size = 8   # Not as tall - breaks square assumption!
    n_catch = 2
    max_cycles = 400
    freeze_evaders = True
    ...
    output_dir = f"./results/mappo_generalization_asymmetric_16x8_{current_time}"
    ...
    print(f"\n{'='*60}")
    print("MAPPO HARD GENERALIZATION TEST: ASYMMETRIC GRID (16x8)")
    print('='*60)
    print(f"Grid: {x_size}x{y_size} (RECTANGULAR - trained on 8x8 square!)")
    print(f"Area: {x_size*y_size} cells (vs 64 in training)")
    print(f"Pursuers: {num_agents}, Evaders: {num_evaders}, n_catch: {n_catch}")
    print(f"Freeze evaders: {freeze_evaders}, Max cycles: {max_cycles}")
    print('='*60 + '\n')
