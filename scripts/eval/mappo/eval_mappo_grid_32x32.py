...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - EXTREME GRID 32x32)")
        print('='*60)
...
if __name__ == "__main__":
    # EXTREME Generalization Test: 32x32 Grid (16x larger area!)
    num_agents = 2
    num_evaders = 1
    x_size = 16  # 4x wider
    y_size = 16  # 4x taller
    n_catch = 2
    max_cycles = 500  # Much longer episodes needed
    freeze_evaders = True
    ...
    output_dir = f"./results/mappo_generalization_grid_32x32_{current_time}"
    ...
    print(f"\n{'='*60}")
    print("MAPPO EXTREME GENERALIZATION TEST: 32x32 GRID")
    print('='*60)
    print(f"Grid: {x_size}x{y_size} (16x larger than training!), Max cycles: {max_cycles}")
    print(f"Pursuers: {num_agents}, Evaders: {num_evaders}, n_catch: {n_catch}")
    print(f"Freeze evaders: {freeze_evaders}")
    print(f"Observation window: 7x7 (same as training)")
    print('='*60 + '\n')
