...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - 3 MOVING EVADERS)")
        print('='*60)
...
if __name__ == "__main__":
    # HARD Generalization Test: 5 Moving Evaders (multi-target + movement)
    num_agents = 2
    num_evaders = 5  # KEY DIFFERENCE: 3 evaders instead of 1
    x_size = 8
    y_size = 8
    n_catch = 2
    max_cycles = 250  # Longer episodes needed
    freeze_evaders = False  # KEY DIFFERENCE: evaders move!
    ...
    output_dir = f"./results/mappo_generalization_5moving_evaders_{current_time}"
    ...
    print(f"\n{'='*60}")
    print("MAPPO HARD GENERALIZATION TEST: 3 MOVING EVADERS")
    print('='*60)
    print(f"Grid: {x_size}x{y_size}, Max cycles: {max_cycles}")
    print(f"Pursuers: {num_agents}, Evaders: {num_evaders} (3x training!)")
    print(f"Freeze evaders: {freeze_evaders} (MOVING!)")
    print(f"n_catch: {n_catch}")
    print(f"Challenge: Multi-target tracking + dynamic movement")
    print('='*60 + '\n')
