...
def get_evader_position(obs: np.ndarray):
    """Get position of evader in local observation."""
    evader_layer = obs[:, :, 2]
...
def moves_toward_evader(obs: np.ndarray, action: int) -> bool:
    """Check if action moves agent closer to evader."""
    h, w = obs.shape[:2]
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - MOVING EVADERS)")
        print('='*60)
...
if __name__ == "__main__":
    # Generalization Test: Moving Evaders
    # Matches: --x_size 8 --y_size 8 --max_cycles 150 --n_pursuers 2 --n_evaders 1 --n_catch 2 (no --freeze_evaders)
    num_agents = 2
    num_evaders = 1
    x_size = 8
    y_size = 8
    ...
    output_dir = f"./results/mappo_generalization_moving_evaders_{current_time}"
    ...
    print(f"\n{'='*60}")
    print("MAPPO GENERALIZATION TEST: MOVING EVADERS")
    print('='*60)
