#!/usr/bin/env python
"""Evaluate a MAPPO checkpoint with recolored observation-built images.

This script mirrors eval_pursuit_mappo_obs_images.py but swaps the colors used
...
class ColorShiftObservationImageWrapper(BaseParallelWrapper):
    ...
    def _maybe_crop(self, local_obs: np.ndarray) -> np.ndarray:
        """Center crop or pad to a target logical grid size."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate MAPPO checkpoint with recolored observation images."
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
    ...
    parser.add_argument(
        ...
        "(default: results/mappo_color_shift_obs_images_<timestamp>).",
    )
    parser.add_argument(
        "--pursuer-color",
        type=_parse_color,
        default="0,220,220",
        help="RGB for pursuers (comma-separated, default: 0,220,220).",
    )
    parser.add_argument(
        "--evader-color",
        type=_parse_color,
        default="255,140,0",
        help="RGB for evaders (comma-separated, default: 255,140,0).",
    )
    parser.add_argument(
        "--wall-color",
        type=_parse_color,
        default="60,60,60",
        help="RGB for walls (comma-separated, default: 60,60,60).",
    )
    parser.add_argument(
        "--background-color",
        type=_parse_color,
        default="245,245,245",
        help="RGB for background (comma-separated, default: 245,245,245).",
    )
    parser.add_argument(
        "--ego-marker-color",
        type=_parse_color,
        default="160,32,240",
        help="RGB for the ego marker (comma-separated, default: 160,32,240).",
    )
    parser.add_argument(
        "--obs-crop-size",
        type=int,
        ...
        "center crop if larger, pad if smaller. Set 0 (default) to match training and disable cropping.",
    )
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - COLOR-SHIFTED OBS IMAGES)")
        print("=" * 60)
...
def _infer_cell_scale_from_checkpoint(checkpoint_dir: str) -> Optional[int]:
    """Try to recover the stride/kernel used during training from the checkpoint."""
    candidate_paths = [
    ...
    for path in candidate_paths:
        ...
        if conv_filters:
            ...
            try:
                # conv_filter format: [out_channels, [kh, kw], stride]
...
def main():
    ...
    # Use the training env name so PPO.from_checkpoint finds a registered creator.
    ...
    # Register under both the training name and a color-shift alias for convenience.
    tune.register_env(env_name, env_creator)
    tune.register_env("pursuit_env_obs_images_color_shift", env_creator)
    ...
    output_dir = args.output_dir or f"./results/mappo_color_shift_obs_images_{current_time}"
    ...
    print(f"\n{'='*60}")
    print("MAPPO GENERALIZATION TEST: COLOR-SHIFTED OBS IMAGES")
    print("=" * 60)
    print(
    ...
    if obs_crop_size and obs_crop_size > 0:
