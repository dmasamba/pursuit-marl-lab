#!/usr/bin/env python
"""Evaluate a MAPPO checkpoint with the ego agent shifted off-center in observations.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate MAPPO checkpoint with the ego agent shifted off-center in the "
    ...
    parser.add_argument(
        ...
        "(default: results/mappo_non_centered_ego_obs_images_<timestamp>).",
    )
    parser.add_argument(
        "--obs-crop-size",
        "--obs_crop_size",
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str, ego_row_offset: int, ego_col_offset: int):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - NON-CENTERED EGO)")
        print("=" * 60)
...
def main():
    ...
    tune.register_env("pursuit_env_obs_images_non_centered_ego", env_creator)
    ...
    output_dir = (
        args.output_dir
        or f"./results/mappo_non_centered_ego_obs_images_{current_time}"
    )
    ...
    print(f"\n{'='*60}")
    print("MAPPO GENERALIZATION TEST: NON-CENTERED EGO")
    print("=" * 60)
