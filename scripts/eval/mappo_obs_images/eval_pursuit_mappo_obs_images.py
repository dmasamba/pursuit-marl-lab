#!/usr/bin/env python
"""Evaluate a MAPPO checkpoint trained with train_pursuit_mappo_obs_images.py."""
...
# under the same module path that was used when the checkpoint was created.
from scripts.train import train_pursuit_mappo_obs_images as train_mod


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate MAPPO checkpoint trained on observation-built images."
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
    ...
    parser.add_argument(
        ...
        "(default: results/mappo_eval_obs_images_<timestamp>).",
    )
...
class MetricsAggregator:
    ...
    def save_summary(self, path: str):
        ...
        print(f"\n{'='*60}")
        print("SUMMARY METRICS (MAPPO - BASELINE OBS IMAGES)")
        print("=" * 60)
...
def main():
    ...
    if args.checkpoint:
        ...
        rl_mod_spec = RLModuleSpec(
            ...
            model_config=DefaultModelConfig(
                conv_filters=train_mod.build_conv_filters(args.cell_scale),
    ...
    output_dir = (
        args.output_dir or f"./results/mappo_eval_obs_images_{current_time}"
    )
    ...
    print(f"\n{'='*60}")
    print("MAPPO EVALUATION: BASELINE OBS IMAGES")
    print("=" * 60)
    ...
    for ep in range(num_eval_episodes):
        ...
        while not done["__all__"]:
            ...
            if args.render:
                # ParallelPettingZooEnv.render forwards an extra argument; call underlying env directly.
